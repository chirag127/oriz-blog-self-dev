...
/** Map a raw category string → one of the five verticals (field-log strata). */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(wellbeing|health|sleep|energy|rest|burnout|body|medic)/.test(c)) return 'health'
  if (/(focus|attention|procrastin|deep work|distraction|discipline|money|finance)/.test(c))
    return 'finance'
  if (/(system|goal|planning|process|routine|method|productiv|tool|engineer|tech|career)/.test(c))
    return 'dev'
  if (/(mindset|motivation|identity|belief|psychology|culture|reflection|self)/.test(c))
    return 'culture'
  return 'personal'
}
